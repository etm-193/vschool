...
function Person(firstName, lastName, age, gender, location) {
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  form.addEventListener('submit', function (event) {
    ...
    const dietaryCheckboxes = form.elements['dietaryRestrictions[]'];
    for (let i = 0; i < dietaryCheckboxes.length; i++) {
      if (dietaryCheckboxes[i].checked) {
        dietaryRestrictions.push(dietaryCheckboxes[i].value);
      }
    }

    const traveler = new Person(firstName, lastName, age, gender, location);
    traveler.dietaryRestrictions = dietaryRestrictions;

    alert(`Thank you for submitting your travel plans, ${traveler.firstName} ${traveler.lastName}!
    Traveler age: ${traveler.age}.
    Traveler gender: ${traveler.gender}.
    Traveler destination: ${traveler.location}.
    Traveler dietary restrictions: ${traveler.dietaryRestrictions.join(', ')}.
    `);
  });
});